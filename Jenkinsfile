        ...
        stage('Sonar') {
            steps {
                withCredentials([usernamePassword(credentialsId: '51fc6c9c-7764-40e5-af76-e88a00b57aad', passwordVariable: 'token', usernameVariable: 'username')]) {
                    bat "mvnw.cmd sonar:sonar -Dsonar.host.url=https://sonarcloud.io -Dsonar.login=${token} -Dsonar.organization=${username}-github -Dsonar.projectKey=${username}_improbable-bot -Dsonar.projectKey=${env.BRANCH_NAME}"
                }
            }
        }
        stage('Archive') {
